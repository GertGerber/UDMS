...
function show_dialog_menu() {
    dialog --clear \
           ...
           1 "Run Uptime-Kuma Installer (Installation Script)" \
           2 "Run CheckMK Installer (Installation Script)" \
           3 "Run Grafana Installer (Installation Script)" \
           4 "Back To Docker Menu" 2>"${INPUT}"

    menu_choice=$(<"${INPUT}")
    case $menu_choice in
        1)
            bash $ROOT_FOLDER/$SCRIPT_FOLDER/$INSTALLER_FOLDER/$DOCKER_MONITORING_FOLDER/$UPTIME_KUMA
            ;;
        2)
            bash $ROOT_FOLDER/$SCRIPT_FOLDER/$INSTALLER_FOLDER/$DOCKER_MONITORING_FOLDER/$CHECKMK
            ;;
        3)
            bash $ROOT_FOLDER/$SCRIPT_FOLDER/$INSTALLER_FOLDER/$DOCKER_MONITORING_FOLDER/$GRAFANA
            ;;
        *)
            exit 0
            ;;
