...
cd
# Script to configure and start a Docker container with Docker Registry

echo -e "${Green}Starting Docker Registry Docker configuration script.${NC}"

echo -e "${Yellow}Do you want to learn more about how to set it up? (Y/N)${NC}"
read -p "Enter your choice: " decision

if [ "$decision" == "Y" ] || [ "$decision" == "y" ]; then
    echo -e "${Yellow}Set up instructions: $DOCKER_REGISTRY_HELPLINK${NC}"
...
read -p "Enter the Docker image for Docker Registry (e.g., registry): " IMAGE
IMAGE=${IMAGE:-"registry:2"}

echo -e "${Green}This step can be skipped if you don't want any changes to the default settings${NC}"
read -p "Enter the name for the Docker Registry container: " CONTAINER_NAME
CONTAINER_NAME=${CONTAINER_NAME:-"Registry"}

echo -e "${Green}This step can be skipped if you don't want any changes to the default settings${NC}"
read -p "Enter the port to expose Docker Registry on (e.g., 5000): " PORT
PORT=${PORT:-5000}
...
# Prompt user to remember to add local mirror setting to /etc/docker/daemon.json
dialog --clear --backtitle "Remember to add the local setting" --title "$SCRIPT_FILENAME" --msgbox "Add the following setting to /etc/docker/daemon.json on your local machine and any other machine that you want to access this registry from:
{
