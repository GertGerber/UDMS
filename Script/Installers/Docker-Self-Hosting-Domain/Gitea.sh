...
cd
# Script to configure and start a Docker container with Gitea

echo -e "${Green}Starting Gitea Docker configuration script.${NC}"

echo -e "${Yellow}Do you want to learn more about how to set it up? (Y/N)${NC}"
read -p "Enter your choice: " decision

if [ "$decision" == "Y" ] || [ "$decision" == "y" ]; then
    echo -e "${Yellow}Set up instructions: $GITEA_HELPLINK${NC}"
...
read -p "Enter the Docker image for Gitea (e.g., gitea/gitea:1.21.7): " IMAGE
IMAGE=${IMAGE:-"gitea/gitea:1.21.7"}

echo -e "${Green}This step can be skipped if you don't want any changes to the default settings${NC}"
read -p "Enter the name for the Gitea container: " CONTAINER_NAME
CONTAINER_NAME=${CONTAINER_NAME:-"Gitea"}

echo -e "${Green}This step can be skipped if you don't want any changes to the default settings${NC}"
read -p "Enter the port to expose Gitea on (e.g., 3000): " PORT
PORT=${PORT:-3000}
...
echo -e "${Green}This step can be skipped if you don't want any changes to the default settings${NC}"
read -p "Enter the port to expose Gitea on (e.g., 222): " PORT2
PORT2=${PORT2:-222}

# Check if the port is already in use by docker
if [[ $(sudo docker ps -a -q -f "publish=$PORT2" | wc -l) -gt 0 ]]; then
    echo -e "${Red}The port $PORT2 is already in use by another Docker container.${NC}"
    read -p "Enter a different port: " PORT2
fi
...
echo -e "${Yellow}This step can't be skipped${NC}"
read -s -p "Enter the database root user password: " DB_PASS_ROOT

# Ensure the user inputs a password
while [ -z "$DB_PASS" ]; do
    echo -e "${Red}A database root user password is required.${NC}"
    read -s -p "Enter the database root user password: " DB_PASS_ROOT
done

echo -e "${Green}This step can be skipped if you don't want any changes to the default settings (gitea)${NC}"
